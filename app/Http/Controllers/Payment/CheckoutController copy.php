...
class CheckoutController extends Controller
{
    ...
    public function guest_checkout()
    {
        if (Cart::instance('default')->count() > 0) {
            ...
            // Retrieve additional details for the checkout
            $deliveryOption = request()->input('deliveryOption');
            $includeCoverLetter = request()->input('includeCoverLetter');
            $totalPrice = request()->input('totalPrice');
    ...
    public function gueststore(Request $request)
{
    try {
        ...
        if (Auth::check()) {
            return redirect()->route('frontend.checkout', app()->getLocale());
        }
        ...
        $data['includeCoverLetter'] = $includeCoverLetter;
            ...
            foreach ($items2->getProduct as $key3 => $items){
                ...
                if(!empty($items2->variants)){  
                    ...
                    foreach($variants as $v => $v_itmes )
                    {
                        ...
                        $item[] =  (new InvoiceItem())->title($title)->pricePerUnit($v_itmes['price']);
                    }
                }else
                {
                    if($items->is_free == '1'){
                    ...
                    $item[] =  (new InvoiceItem())->title($items->name)->pricePerUnit($pricePerUnit);
                }
            }
