...
class UserController extends Controller
{
	...
	public function signup(Request $request)
	{
		...
		if ($user)
			return json_response(['status' => true, 'url' => route('frontend.sign-in'), 'msg' => trans('frontend_msg.succ_signup_with_email_verify')], 200);
		else
	...
	public function updateProfile(Request $request)
	{
		...
		return json_response([
			'status' => true,
			'msg' => trans('frontend_msg.succ_update_profile'),
	...
	function update_user_image(Request $request)
	{
		...
		return json_response([
			'status' => true,
			'msg' => trans('frontend_msg.succ_update_user_image'),
			'url' => route('frontend.profile')
		], 200);
			}
	...
	public function sendMail($request,$type){
		...
		if($setting->is_checked_smtp == 1){ //if user have set smtp credentails
			...
			if($type == 'registration'){
				...
				$link_text = '<a href="'.route('frontend.email-verify', ["token" => $token]).'">'.$setting->reg_link_text.'</a>';
			...
			elseif($type == 'forget_password'){
				...
				$link_text = '<a href="'.route('frontend.reset-password', ["token" => $token]).'">'.$setting->forget_pass_link_text.'</a>';
			...
			{
				$final_template .=  '<img src="'.\Storage::url(getsetting()->my_logo).'" alt="logo" width="200px" height="50px" > </br> </br>'."\r\n";
			}
