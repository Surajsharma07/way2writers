...
<meta property="og:locale" content="en_US" />
    ...
    <!-- Favicons -->
    <link href="{{ $STORAGE_URL . @$setting->favicon_img }}" rel="icon">
...
var BASE_URL = "{{ url('/') }}/";
        $('.tp_banner_section').css('background-image', 'url({{ $STORAGE_URL . @$setting->home_page_bg_img }})');
        $('.tp_TopSelling_section').css('background-image', 'url({{ $STORAGE_URL . @$setting->home_middle_banner }})');

        @if (@$setting->is_checked_animation_bg)
            $('.tp_bnnner_section2').css('background-image',
                'url({{ $STORAGE_URL . @$setting->home_page_bg_animation_img }})');
        @endif
        var HAPPY_STRIKER = "{{ $STORAGE_URL . @$setting->success_icon_img }}"
        var SAD_STRIKER = "{{ $STORAGE_URL . @$setting->error_icon_img }}"
    </script>
