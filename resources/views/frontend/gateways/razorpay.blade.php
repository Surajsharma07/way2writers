...
<a href="{{ route('frontend.checkout') }}" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></a>
         ...
         <div class="text-center">
             <form action="{{ route('razorpay.make.payment') }}" method="POST">
                 ...
                 <script src="https://checkout.razorpay.com/v1/checkout.js" 
                     ...
                     data-image="{{ Storage::url(getsetting()->my_logo) }}" 
                     data-prefill.name="{{ auth()->user()->full_name }}"
                     data-prefill.email="{{ auth()->user()->email }}" data-theme.color="#216fff"
                     ></script>
             </form>
         </div>
     </div>
