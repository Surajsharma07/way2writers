...
@section('content')
    ...
    <div class="tp_singlepage_section tp_cart_wrapper">
        <div class="container">
                    ...
                    <div class="tp_view_box">
                        ...
                        @if (Cart::instance('default')->count() > 0)
                            <div class="tp_cart_step">
                                ...
                                <div class="tp_step_box">
                                    <a href="{{ route('frontend.checkout') }}"><svg
            ...
            <p>
                Delivery Option: {{ $item->options->delivery_option ?? '' }}:
            </p>
            <p>
                {{ $price_symbol }}{{ $item->options->delivery_option_price ?? 0 }}
            </p>
        </div>

        <!-- Display Include Cover Letter and Price -->
        @if ($item->options->include_cover_letter)
            <p>
                <strong>{{ $item->options->include_cover_letter ? __('Cover Letter') : __('master.cart.no') }}:</strong>
            </p>
            <p>
               {{ $price_symbol }}{{ $item->options->cover_letter_price ?? 0 }} 
            </p>
            @endif
</div>
...
<div class="addto_cart">
    <h4>
        Total: {{ $price_symbol }}{{ number_format($item->subtotal, 2) }}
                                ...
                                <div class="tp_overlay_btn">
                                    <form
action="{{ route('frontend.cart.destroy', [$item->rowId, 'default']) }}"
        ...
        <div class="tp_view_text"></div>
        <div class="tp_continue_checkout">
            <a @if (Auth::check()) href="{{ route('frontend.checkout') }}" @else data-bs-toggle="modal"
                data-bs-target="#log_modal" @endif class="tp_btn">
                @lang('master.cart.continue_to_checkout')
                ...
                @else
                    <p>@lang('master.cart.no_item_cart') <a
                            href="{{ route('frontend.home') }}">@lang('master.cart.add_now')</a></p>
                @endif
            </div>
        </div>
    </div>
