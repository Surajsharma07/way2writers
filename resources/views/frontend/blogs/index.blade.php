...
<h3> Blogs </h3>                    <div style="font-size: 16px; font-weight: 600; margin-top: 0px; margin-bottom: 16px; width: 50%; word-wrap: break-word;">{!! @$product->short_desc !!}</div> 
    ...
    <div class="row">
      ...
      <div class="col-md-9 blog-row">
        ...
        <div class="blog-column">
          <div class="blog-item">
          <a href="{{ route('frontend.blogs.show', ['slug' => $blog->slug]) }}">
            <img src="{{ asset('storage/' . $blog->image) }}" alt="Blog Image" class="blog-img">
            <div class="blog-item-content bg-white p-4">
<div class="blog-item-meta  py-1 px-2">
					<span class="text-muted text-capitalize mr-3"><i class="bi bi-pencil-square mr-2"></i>{{ $blog->category->name }}</span>
				</div> 
                <h4 class="mt-3 mb-3">{{ $blog->name }}</h4>
           <p class="mb-4">{!! Str::limit(strip_tags($blog->description), 150) !!}</p>
</a>
        ...
        <ul>
          @foreach ($recentPosts as $blog)
            <li class="recent-post-item border-bottom py-3">
                      <a href="{{ route('frontend.blogs.show', ['slug' => $blog->slug]) }}">
                <img src="{{ asset('storage/' . $blog->image) }}" alt="Recent Post Image" class="recent-post-img">         
            <div class="media-body">
                <h6 class="my-2 font-weight-bold"><a href="{{ route('frontend.blogs.show', ['slug' => $blog->slug]) }}">{{ $blog->name }}</a></h6></a>
                <span class="text-sm text-muted">{{ $blog->created_at->format('d-M-Y')  }}</span>
          @endforeach
            </div>
            </li>
        </div>
