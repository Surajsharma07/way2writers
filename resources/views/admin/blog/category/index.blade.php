...
@section('content')
    <div class="tp_main_content_wrappo">
        <div class="tp_tab_wrappo">
            <ul>
                <li><a href="{{ route('admin.blog-category.create') }}">Add Category</a></li>
                <li class="active"><a href="{{ route('admin.blog-category.index') }}">Manage Categories</a></li>
            </ul>
        </div>
                                    ...
                                    @if ($data->count() > 0)
                                        @foreach ($data as $key => $item)
                                            <tr id="table_row_{{ $item->id }}">
                                                    ...
                                                    <div class="tp_autoresponder">
                                                        <label class="tp_toggle_label">
                                                            <input id="active_btn_{{ $item->id }}" type="checkbox"
                                                                name="status" value="1"
                                                                onclick="update_single_status('{{ route('admin.blog-category.update', $item->id) }}','{{ $item->is_active }}','{{ 'active_btn_' . $item->id }}')"
                                                ...
                                                <td>
                                                    <ul>
                                                        <li><a href="{{ route('admin.blog-category.edit', $item->id) }}"
                                                        ...
                                                        <li><a href="#" class="tp_delete tp_tooltip" title="Delete"
                                                                onclick="delete_single_details('{{ route('admin.blog-category.destroy', $item->id) }}',{{ $item->id }})"><i
