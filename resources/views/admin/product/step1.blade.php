...
@section('content')
    <div class="tp_main_content_wrappo">
        <div class="tp_tab_wrappo">
            <h4 class="tp_heading">@if(isset($data->id)) Edit @else Add @endif Products (Step 1)</h4>
            @if (isset($data->id) && !@empty($data->id))
                <div class="tp_form_wrapper ">
                    <ul>
                        <li class="active"><a href="{{ route('admin.product.edit', ['id' => $data->id]) }}">Edit (Step 1)</a></li>
                            ...
                            <form action="{{ route('admin.product.store.step1') }}" id="product-first-step-form"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 col-md-12">
                                        ...
                                        <input type="text" class="form-control" name="slug" placeholder="Enter URL Name (Slug)"
                                            value="{{ @$data->slug }}">
                                    </div>
                                    </div>
            ...
            <label id="category_id-error" class="error" for="category_id"></label>
                                ...
                                <div class="col-lg-12 col-md-12">
                                    <div class="tp_form_wrapper tp_form_textarea">
                                        ...
                                        <textarea rows="5" cols="50" spellcheck="true" class="form-textarea" placeholder="Enter Short Description" name="short_desc">{{ @$data->short_desc }}</textarea>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-md-12">
                                    <div class="tp_form_wrapper tp_form_textarea">
                                        <label class="mb-2">Tags<sup>*</sup></label>
                                        <textarea rows="5" cols="50" spellcheck="true" class="form-textarea" placeholder="Enter Tags" name="tags" placeholder="Separate each tag by comma (,)">{{ @$data->tags }}</textarea>
                                    </div>
                                </div>
                        <div class="col-lg-12">
    <div class="tp_form_wrapper tp_form_mb">
        <label class="mb-2">New Description<sup>*</sup></label>
        <textarea id="tinymce-editor"  name="description" placeholder="Enter HTML content here">{{ @$data->description }}</textarea>
    </div>
</div>
...
<script>
  tinymce.init({
    ...
    toolbar: 'code undo redo | blocks fontfamily fontsize | bold italic underline strikethrough | link image media table mergetags | align lineheight | checklist numlist bullist indent outdent | emoticons charmap',
    tinycomments_mode: 'embedded',
    menubar: '',
     extended_valid_elements: 'span',
    content_css: 'http://localhost/newway/public/user-theme/my_assets/extra.css',
    ai_request: (request, respondWith) => respondWith.string(() => Promise.reject("See docs to implement AI Assistant")),
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="checkbox">
                                            <label><input type="checkbox" value="1" class="form-control"
                                                    name="is_featured" @if (@$data->is_featured == 1) checked @endif><i
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="checkbox">
                                            <label><input type="checkbox" value="1" class="form-control"
                                                    name="is_active" @if (@$data->is_active == 1) checked @endif><i
                                                    class="input-helper"></i>Yes</label>
                                        </div>
                                        </div>
                                    </div>
                                ...
                                <div class="tp_form_wrapper">
                                    ...
                                    <input type="text" class="form-control" placeholder="Enter Live Preview Link" name="preview_link" placeholder="On ssl server non ssl link will not show in iframe"
                                        ...
                                        <div id="p-d-body">
                                            @if (@$data->product_details[0])
                                                @foreach (@$data->product_details as $key => $val)
                                                    <div class="row align-items-center" id="p-field">
                                                        <div class="col-md-5">
                                                            <label class="mb-2">Key</label>
                                                            <input type="text" class="form-control" placeholder="Enter Key" name="product_key[]"
                                                                value="{{ @$val['key'] }}" placeholder="Enter Key">
                                                        </div>
                                                        <div class="col-md-5">
                                                            <label class="mb-2">Value</label>
                                                            <input type="text" class="form-control" placeholder="Enter Value" name="product_value[]"
                                                                value="{{ @$val['value'] }}" placeholder="Enter value">
                                                        </div>
                                                        <div class="col-md-2">
                                                            <div class="field-button add_btn">
                                                            @if ($loop->first)
                                ...
                                @if (isset($data->id) && !@empty($data->id))
                                    <button id="product-first-step-form-btn" type="submit" class="tp_btn">Update (Step 1)</button>
                                @else
                                    <button id="product-first-step-form-btn" type="submit" class="tp_btn">Add (Step 1)</button>
                                @endif
                            </form>
                        </div>
                    </div>
